...
public class MeiJuCrawlerExecutor {
    ...
    private final static String PRE_URL = "http://www.msj1.com/c/";
    private final static String END_URL = "/page/";
    private final static int CRAWLER_PAGE_SIZE = 3;
    ...
    public void execute() {
        ...
        List<Menu> menuList = menuService.getMeiJuMenu();
        for (Menu menu : menuList) {
            for (int i = CRAWLER_PAGE_SIZE; i >= 1; i--) {
                String content = crawlerWorker.getResponseContent(PRE_URL + menu.getUrl() + END_URL + i);
                ...
                meiJuSimples.forEach(meiJuSimple -> {
                    ...
                    if (meiJu.downLinksNotNull()) {
                        ...
                        LOG.info(meiJu.getTid() + " -- " + meiJu.getImage());
                    }
                });
            }
        }
